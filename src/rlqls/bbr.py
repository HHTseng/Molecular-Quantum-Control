...
def rate_generator(rates_i_to_j: np.ndarray) -> np.ndarray:
    """Build the column-vector master-equation generator ``ds/dt=G s``.

    Input ``rates_i_to_j[i,j]=R_(i->j)`` gives ``G[j,i]=R_(i->j)`` off the
    diagonal and ``G[i,i]=-sum_(j!=i)R_(i->j)`` (pseudocode Sec. 4).
    """
    r = np.asarray(rates_i_to_j, dtype=np.float64)
    if r.ndim != 2 or r.shape[0] != r.shape[1] or np.min(r) < 0:
        raise ValueError("rates must be nonnegative square matrix")
    r = r.copy()
...
def propagator(
    generator: np.ndarray, duration_s: float, *, method="exact", microstep_s: float = 1e-6
) -> np.ndarray:
    """Return ``T_BBR(Delta t)`` so the noisy population is ``s'=T_BBR s``.

    ``exact`` uses ``exp(G Delta t)``; ``paper_euler`` uses
    ``(I+G delta t)^M`` with ``M=ceil(Delta t/delta t)``.
    """
    ...
    if method == "exact":
        ...
        count = max(1, int(np.ceil(duration_s / microstep_s)))
        dt = duration_s / count
...
def action_propagators(generator: np.ndarray, durations_ms: np.ndarray, **kwargs) -> np.ndarray:
    """Precompute one ``T_BBR(tau_a)`` for every pulse duration ``tau_a``."""
